...
int main(int argc, char **argv)
{
    ...
    cv::rectangle(image_freeze, cv::Point(xmin, ymin), cv::Point(xmax, ymax), cv::Scalar(0, 255, 0), 1);
    cv::imshow("Output", image_freeze);
    cv::waitKey(0);

    cv::imwrite("../images/output/identificado/exp_cc.png", image_freeze);
    ...
    cv::waitKey(0);
    cv::imwrite("../images/output/placa/exp_cc_plate.png", roi);
