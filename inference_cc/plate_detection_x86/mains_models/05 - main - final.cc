...
int main(int argc, char **argv)
{
    ...
    while (true) {
        ...
        // ##################### Image Detection ##################### 
            ...
            // cv::rectangle(frame, cv::Point(xmin, ymin), cv::Point(xmax, ymax), cv::Scalar(0, 255, 0), 1);
            // cv::imshow("cam", frame);
            // cv::waitKey(frameDuration);
            ...
            std::string str1 = std::string("ERM5431");

            if (outText == str1){
                std::cout << "Passagem autorizada";
            }
            ...
            cv::putText(frame, "FPS: " + std::to_string(fpsLive), {50,50}, cv::FONT_HERSHEY_SIMPLEX, 1.5, (255,255,255));
            cv::imshow("cam", frame);
            cv::waitKey(1);
            // cv::waitKey(frameDuration);
